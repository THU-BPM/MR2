...
class BertLSTM(nn.Module):
    def __init__(self, settings, clip_model=None):
        ...
        self.dropout_in = settings['dropout_in'] 

        self.lstm = torch.nn.LSTM(self.input_size,self.hidden_size, self.num_layers, dropout=self.dropout,bidirectional =self.bidirectional, batch_first=True )
        self.pool_layer = nn.AdaptiveAvgPool1d(1)
        print(self.bidirectional)
        
    def forward_evidence(self, sentence_to_embed):
        batch_size = sentence_to_embed.size(0)
        evidence_size = sentence_to_embed.size(1)
        seq_len = sentence_to_embed.size(2)
        bert_dim = sentence_to_embed.size(3)
        hidden_size = (self.hidden_size*2) if self.bidirectional else self.hidden_size
        sentence_to_embed = torch.reshape(sentence_to_embed, (batch_size*evidence_size,seq_len,bert_dim))
        sentence_to_embed = F.dropout(sentence_to_embed,self.dropout_in)   

        output, (last_hidden, _) = self.lstm(sentence_to_embed)
        output = torch.reshape(output, (output.size(0),output.size(2),output.size(1)))
        output = self.pool_layer(output)
        
        output = torch.reshape(output, (batch_size, evidence_size, hidden_size))

        last_hidden = torch.reshape(last_hidden, (last_hidden.size(1), last_hidden.size(0), last_hidden.size(2)))
        last_hidden = torch.reshape(last_hidden, (last_hidden.size(0), hidden_size))
        last_hidden = torch.reshape(last_hidden, (batch_size, evidence_size, hidden_size)) 

        combined_output = torch.cat((output,last_hidden),dim=2) 
        return combined_output 

    def forward_query(self, sentence_to_embed):
        batch_size = sentence_to_embed.size(0)
        seq_len = sentence_to_embed.size(1)
        bert_dim = sentence_to_embed.size(2)
        ...
        output = torch.reshape(output, (output.size(0),output.size(2),output.size(1)))
        output = self.pool_layer(output)
        output = torch.reshape(output, (batch_size, hidden_size))
        last_hidden = torch.reshape(last_hidden, (last_hidden.size(1), last_hidden.size(0), last_hidden.size(2)))
        last_hidden = torch.reshape(last_hidden, (last_hidden.size(0), hidden_size))
        
        combined_output = torch.cat((output,last_hidden),dim=1) 
        return combined_output 
        
class ContextMemNet(nn.Module):
    def __init__(self, settings, lstm_settings):
        ...
        self.captions_lstm = BertLSTM(lstm_settings)
        ...
        #Entities memory 
        self.entities_lstm = BertLSTM(lstm_settings)
        ...
        self.query_lstm = BertLSTM(lstm_settings)
    ...
    def add_binary_features(self,binary_features, results):
        results = torch.cat( (results,binary_features), dim=-1) 
        return results
        
    def forward_query_mem(self,query_img,query_captions,query_places=None):  
        ...
        query_captions = self.query_lstm.forward_query(query_captions)
        query_captions = F.dropout(query_captions, self.inp_pdrop)
        return query_img_proj,query_captions,query_places_proj
    ...
    def consistency_clip(self,qimage_clip, qtext_clip):
        ###we will read precomputed embeddings 
    ...
    def forward(self,query_img, query_captions, query_places=None, entities_bert=None, entities_binary=None, results_images=None, results_places=None, images_domains=None, results_captions_bert=None, captions_domains=None, captions_binary=None, qimage_clip=None, qtext_clip=None):
        ...
        query_projections = self.forward_query_mem(query_img,query_captions,query_places)

        #lstms for entities and captions:
        #add binary indicators to the lstm output 
        if self.use_ent_memory:
            entities = self.entities_lstm.forward_evidence(entities_bert) 
            if self.binary_feature_ent: entities = self.add_binary_features(entities_binary, entities)
        if self.use_cap_memory:
            results_captions = self.captions_lstm.forward_evidence(results_captions_bert)  
            if self.binary_feature_cap: results_captions = self.add_binary_features(captions_binary,results_captions)            
        ...
        if self.use_img_memory:
            img_mem_out,P_img = self.generic_memory(query_projections[0], results_images, self.W_im_a, self.W_im_c,self.bn_imgMem, self.img_mem_hops,self.Wqimg_hop if self.img_mem_hops>1 else None,self.W_dec_img_mem if self.fusion=='byDecision' else None)
            out_features = img_mem_out
        if self.use_cap_memory: 
            cap_mem_out,P_cap = self.generic_memory(query_projections[1], results_captions, self.W_sent_a, self.W_sent_c, self.bn_capMem, self.cap_mem_hops,self.Wqcap_hop if self.cap_mem_hops>1 else None,self.W_dec_cap_mem if self.fusion=='byDecision' else None)
            out_features = torch.cat((out_features, cap_mem_out),dim=-1)
        if self.use_ent_memory:
            ent_mem_out,P_ent = self.generic_memory(query_projections[1], entities, self.W_ent_a, self.W_ent_c, self.bn_entMem, self.ent_mem_hops,self.Wqent_hop if self.ent_mem_hops>1 else None,self.W_dec_ent_mem if self.fusion=='byDecision' else None)
            out_features = torch.cat((out_features, ent_mem_out),dim=-1)
        if self.use_places_memory:
            places_mem_out,P_places = self.generic_memory(query_projections[2], results_places, self.W_places_a, self.W_places_c, self.bn_placesMem, self.places_mem_hops,self.Wqplaces_hop if self.places_mem_hops>1 else None,self.W_dec_places_mem if self.fusion=='byDecision' else None)
            out_features = torch.cat((out_features, places_mem_out),dim=-1)
            
        if self.consistency == 'clip':                 
            consistency_out = self.consistency_clip(qimage_clip , qtext_clip)
