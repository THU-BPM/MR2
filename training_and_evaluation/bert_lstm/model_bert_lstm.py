...
class ContextMemNet(nn.Module):
    def __init__(self, settings, lstm_settings):
        ...
        if settings['fusion'] == 'byDecision': 
            ...
            self.W_final_dec =  [nn.Linear(count_dim if l == 0 else self.fc_dims[l-1], self.fc_dims[l] if l != self.nlayers - 1 else 3) for l in range(self.nlayers)]
    ...
    def forward(self,query_img, query_captions, query_places=None, entities_bert=None, entities_binary=None, results_images=None, results_places=None, images_domains=None, results_captions_bert=None, captions_domains=None, captions_binary=None, qimage_clip=None, qtext_clip=None):
        ...
        if self.use_cap_memory: 
            cap_mem_out = self.generic_memory(query_projections[1], results_captions, self.W_sent_a, self.W_sent_c, self.bn_capMem, self.cap_mem_hops,self.Wqcap_hop if self.cap_mem_hops>1 else None,self.W_dec_cap_mem if self.fusion=='byDecision' else None)
            #print(f'img shape is {out_features.shape},cap shape is {cap_mem_out.shape}')
            out_features = torch.cat((out_features, cap_mem_out),dim=-1)
        if self.use_ent_memory:
            ent_mem_out = self.generic_memory(query_projections[1], entities, self.W_ent_a, self.W_ent_c, self.bn_entMem, self.ent_mem_hops,self.Wqent_hop if self.ent_mem_hops>1 else None,self.W_dec_ent_mem if self.fusion=='byDecision' else None)
            out_features = torch.cat((out_features, ent_mem_out),dim=-1)
        if self.use_places_memory:
            places_mem_out = self.generic_memory(query_projections[2], results_places, self.W_places_a, self.W_places_c, self.bn_placesMem, self.places_mem_hops,self.Wqplaces_hop if self.places_mem_hops>1 else None,self.W_dec_places_mem if self.fusion=='byDecision' else None)
